...
画像生成プロンプトや生成画像がしっくりこない場合は、以下のように依頼します。

```text
1枚の中でキャラクターの服装が勝手に変わっています。
衣装は原則固定、構成指示がある場合のみ変更可にしてください。
```

```text
画風が淡い水彩・絵本寄りになっています。
太めの黒い漫画線とSNS解説漫画らしい情報密度を強めてください。
```

```text
少し派手な単発バナー風になっています。
装飾を控えめにし、連載漫画として落ち着いた統一感を優先してください。
```

```text
Gemini風の配色にしたいです。
ただし、キャラクター本体ではなく、背景・見出し帯・UIカード・装飾だけに反映してください。
```
