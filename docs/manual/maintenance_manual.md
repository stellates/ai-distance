...
以下のプロンプトを貼り付ける前に、既存漫画画像を3つ程度添付してください。
例：
...
画像生成元がPNGの場合は、WebP quality 75〜80を目安に変換してから保存します。
既存記事のPNGは変更しません。
...
```bash
git status
git add posts/NNN.html index.html assets/images/NNN-topic.webp
git commit -m "add post NNN"
git push -u origin agent/article-NNN
```

ファイル名や記事番号は、実際に追加したものに置き換えてください。
ローカル確認後に、作業ブランチを `main` へ反映するか人間が判断します。
