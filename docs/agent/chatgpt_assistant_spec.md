...
ChatGPT / 人間補助モードで新記事作成を支援する場合は、作業の現在地を見失わないよう、`docs/agent/workflow.md` も参照します。
...
新記事作成支援では、`docs/agent/workflow.md` に基づき、主要な返信で現在の進行状況を短く表示します。
...
- `AGENTS.md` を確認する
- この `docs/agent/chatgpt_assistant_spec.md` を確認する
- `docs/agent/workflow.md` を確認する
...
ただし、これは概要です。新記事作成支援の実際の進行管理は `docs/agent/workflow.md` を基準にします。
...
- 画像生成プロンプトの標準ルールを反映する

### 基準サンプル画像の確認

画像生成設計時は、シリーズの雰囲気をそろえるため、原則として以下の基準サンプル画像3点を確認対象にします。

- `assets/images/019-ai-site-too-hard.png`
- `assets/images/024-all-is-too-much.png`
- `assets/images/025-sore-kiitenai.png`

ChatGPT が GitHub連携等で画像内容を直接確認できる場合は、これらの画像を参照し、以下を確認します。
...
画像内容を直接確認できない場合は、画像を見た前提で分析しません。
その場合は、人間に上記3画像を画像生成チャットへ添付してもらうか、画像内容の確認を依頼します。

画像生成プロンプトでは、確認済みの基準サンプル画像の雰囲気に合わせるよう明記します。
