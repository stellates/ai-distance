...
- `docs/agent/post_inventory.md`を予め読み込み、これまでの記事や現在の最新話を把握しておく

### 作業ブランチ

- `main` を直接編集せず作業ブランチを使う
...
- ユーザーから提示がなければ、新記事のテーマや構成案決定を補助する
- 画像生成プロンプトの作成およびレビュー、実際の画像生成を行う
- HTML、indexカード、inventoryの差分を作る
- ユーザーのgit操作やlocalファイル操作を補助する
...
- ユーザー指示に `/auto` が含まれる場合は `docs/agent/coding_agent_spec.md` を参照する
- それ以外の場合は 記事追加・更新作業を希望か、それ以外の作業（（AGENTS.md関係の整備、README.mdの修正、機能追加やバグ対応などの改修作業など）を希望かをユーザーに確認する
- ユーザー指示が 記事追加・更新作業を希望している場合は `docs/agent/chatgpt_assistant_spec.md` を参照する
