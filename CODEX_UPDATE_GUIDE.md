...
Codex は `CODING_AGENT_SPEC.md` に従って、ローカル作業ツリー上で新記事作成から `git commit` までを行います。
...
Codex 側の詳細な作業仕様は `CODING_AGENT_SPEC.md` を参照します。
ChatGPT 連携で作業ブランチ運用を行う場合は `UPDATE_GUIDE.md` と `CHATGPT_ASSISTANT_GUIDE.md` を参照します。
...
2. 人間が Codex にテーマを渡す
3. Codex が CODING_AGENT_SPEC.md を読んで作業する
...
```text
AGENTS.md を読んで、CODING_AGENT_SPEC.md に従ってください。
...
```text
push は行わないでください。
CODING_AGENT_SPEC.md の通り、commit までで停止してください。
```
